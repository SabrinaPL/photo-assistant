...
import { PhotoStorage } from '../utils/photoStorage.js'
...
export class PhotoAssistant {
  ...
  addImage (image) {
    try {
      ...
      const newPhoto = photoStorage.runPhotoStorage(image)
  ...
  sortPhotos (image, sortBy) {
    console.log('in sort photos method, photoAssistant')
    console.log(sortBy)
    const photoSort = new PhotoSort(image, sortBy)

    photoSort.runPhotoSort()
  }
  ...
  #createNewGallery (images, columns, galleryContainer) {
    ...
    photoGallery.setupGallery()
    photoGallery.renderGallery(galleryContainer)
  }
}
