...
export class PhotoFilter {
  ...
  constructor (filterMethod, filterValue) {
    if (typeof (filterMethod) !== 'string' || typeof (filterValue) !== 'string' || filterMethod === null || filterValue === null) {
  ...
  // TODO:
  // Break down the entry method into two separate methods:
  // 1. Method add filter effects and value to an array (to keep track)

  #addChosenFilterMethods (filterMethod, filterValue) {
    this.#chosenFilterMethods.push({
      filterMethod,
      filterValue
    })
  }
  ...
  /* if (filterName === null || typeof (filterName) !== 'string' || filterValue === null || typeof (filterValue) !== 'string') {
    throw new Error('Invalid string')
  } */

  // 2. Another method to add the filter effects to the image
